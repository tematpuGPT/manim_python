...
class ColumnMultiplication(Scene):
    def construct(self):
        ...
        # Показываем окончательный результат с анимацией перемещения промежуточных результатов
        self.play(
            ApplyMethod(res1.move_to, final[0:4].get_center()), # перемещаем первый результат на место первых четырех цифр окончательного
            ApplyMethod(res2.move_to, final[1:5].get_center()), # перемещаем второй результат на место вторых четырех цифр окончательного
            ApplyMethod(res3.move_to, final[2:6].get_center()), # перемещаем третий результат на место третьих четырех цифр окончательного
            ApplyMethod(res4.move_to, final[3:7].get_center()), # перемещаем четвертый результат на место четвертых четырех цифр окончательного
            run_time=2 # устанавливаем время анимации в 2 секунды
        )
        self.play(FadeIn(final)) # показываем окончательный результат
        self.wait()